...
func GetItemById(writer http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		...
		msg := models.Message{Error: "do not use parameter ID as uncasted to int type"}
	...
	if !ok {
		...
		msg := models.Message{Error: "Item with that id not found"}
		json.NewEncoder(writer).Encode(msg)
	} else {
...
func CreateItem(writer http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		msg := models.Message{Error: "provideed json file is invalid"}
		writer.WriteHeader(400)
		json.NewEncoder(writer).Encode(msg)
		return
	}
	...
	if err = models.AddItem(item, id); err != nil {
		...
		writer.WriteHeader(400)
		msg := models.Message{Error: fmt.Sprint(err)}
		json.NewEncoder(writer).Encode(msg)
		return
	}
...
func UpdateItemById(writer http.ResponseWriter, request *http.Request) {
	...
	if !ok {
		...
		writer.WriteHeader(404)
		msg := models.Message{Error: "item with that ID does not exists in database"}
		json.NewEncoder(writer).Encode(msg)
		return
	}
